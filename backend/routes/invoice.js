...
router.post("/bills", async (req, res) => {
  try {
    const {  filename, mobileNumber, orderDate, customerName } = req.body;

    if ( !filename || !mobileNumber || !orderDate || !customerName) {
    ...
    doc.addImage(imageBuffer, "PNG", 10, 10, 180, 160); // Adjust dimensions as needed
    doc.save(filePath);
