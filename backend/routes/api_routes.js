...
router.post("/product", upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    const sql = "INSERT INTO products (product_name, description, category, price, stock, image_url) VALUES (?, ?, ?, ?, ?, ?)";
    const values = [product_name, description, category, price, stock, image_url];
...
router.put("/product/:id", upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    const sql = "UPDATE products SET product_name = ?, description = ?, category = ?, price = ?, stock = ?, image_url = ? WHERE id = ?";
    const values = [product_name, description, category, price, stock, image_url, id];
...
router.post("/orders", async (req, res) => {
  try {
    const { customerName, orderDate, products, totalPrice, mobile } = req.body;

    // Insert order details into 'orders' table
    const [orderResult] = await db.query(
        "INSERT INTO orders (customer_name, order_date, total_price , mobile) VALUES (?, ?, ?, ?)",
        [customerName, orderDate, totalPrice, mobile]
    );
...
router.get("/orders", async (req, res) => {
  try {
    ...
    // Fetch all orders
    if(req.query.date) {
      const [result] = await db.query("SELECT * FROM orders WHERE order_date = ?", [req.query.date]);
      orders = result;
    } else {
...
//     const [orders] = await db.query("SELECT * FROM orders WHERE order_date = ?", [date]);
...
router.put("/orders/:id", async (req, res) => {
  const { customerName, orderDate, mobile, products, totalPrice } = req.body;
  const orderId = req.params.id;

  try {
      // Update the main order table
      await db.query(
          "UPDATE `orders` SET `customer_name`=?, `order_date`=?, `mobile`=?, `total_price`=? WHERE `id`=?",
          [customerName, orderDate, mobile, totalPrice, orderId]
      );
